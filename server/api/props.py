...
def build_router(cache: OddsCache) -> APIRouter:
    ...
    @router.get("/api/props/mlb", response_model=OddsResponse)
    async def get_mlb_props() -> OddsResponse:
        now = datetime.now(timezone.utc)
        rows = [r for r in cache.all_current() if is_prop_market(r["market_key"])]
