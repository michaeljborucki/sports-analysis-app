...
from .arbitrage import _best_visible, _find_market, _outcome_point
...
def scan_game_low_hold(
    ...
    for abs_pt, sides in spread_by_abs.items():
        ...
        pair = _try_low_hold_pair(
            sides["home"], sides["away"], books_filter, max_hold_pct
        )
        if pair:
            best_a, best_b, hold = pair
            out.append(
    ...
    for pt, sides in total_by_point.items():
        if "over" not in sides or "under" not in sides:
            continue
        pair = _try_low_hold_pair(
            sides["over"], sides["under"], books_filter, max_hold_pct
        )
        if pair:
            best_a, best_b, hold = pair
            out.append(
