...
def update_model_weights(brier_scores: dict, weights_path: str = None) -> None:
    ...
    Applies sample-size shrinkage: with few bets, weights are pulled toward
    uniform (1.0) to prevent overfitting to small-sample Brier scores.
    """
    from ensemble.weights import load_weights, save_weights, BET_SLOTS, shrink_weights
    ...
    # Apply shrinkage toward uniform based on total settled bets
    try:
        bets_df = load_bets()
        n_settled = len(bets_df[bets_df["result"].isin(["W", "L", "P"])])
    except Exception:
        n_settled = 0

    if n_settled > 0:
        weights = shrink_weights(weights, n_settled)
