...
def test_parse_challenge_approve():
    ...
    assert result["moneyline"]["verdict"] == "approve"
    assert result["moneyline"]["reasoning"] == "looks solid"
    assert result["moneyline"]["flaw_found"] is None
...
def test_parse_challenge_kill():
    ...
    result = parse_challenge_response(raw)
    assert result["moneyline"]["verdict"] == "kill"
    assert result["moneyline"]["flaw_found"] == "overfit"
    assert result["total"]["verdict"] == "approve"
...
def test_run_challenge_success(mock_runner):
    ...
    assert verdicts["moneyline"]["verdict"] == "approve"
    assert cost > 0
...
def test_run_challenge_failure_approves_all(mock_runner):
    ...
    assert verdicts["moneyline"]["verdict"] == "approve"
    assert verdicts["total"]["verdict"] == "approve"
    assert cost == 0
