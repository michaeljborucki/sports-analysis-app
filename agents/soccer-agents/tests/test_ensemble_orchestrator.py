...
def test_classify_consensus_strong():
    """All 6 models agree on home for asian_handicap → strong."""
    ...
    # All predict home for asian_handicap, so should be strong (6 agree >= 5)
    assert classification["asian_handicap"]["level"] == "strong"
...
def test_classify_consensus_no_consensus():
    ...
    home_pred = make_prediction(asian_handicap={"value_side": "home"})
    away_pred = make_prediction(asian_handicap={"value_side": "away"})
    none_pred = make_prediction(asian_handicap={"value_side": "none"})
    ...
    classification = classify_consensus(results, MOCK_ODDS)
    assert classification["asian_handicap"]["level"] == "none"
...
def test_reclassify_consensus_uses_majority_of_runs():
    ...
    assert classification["asian_handicap"]["level"] == "strong"
    assert classification["asian_handicap"]["count"] == 5
