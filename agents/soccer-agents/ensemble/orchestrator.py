...
from config import ENSEMBLE_MODELS, CONSENSUS_MIN_VOTES, MAX_CALLS_PER_GAME, HOME_ADVANTAGE_BY_LEAGUE
...
from ensemble.quant_poisson import synthetic_run_result as quant_poisson_result
...
def _log_all_predictions(results: list[dict], odds: dict, game_label: str) -> None:
    ...
    for r in results:
        ...
        for slot in BET_SLOTS:
            ...
            market_prob = implied.get(f"ah_home", 0.5)  # rough fallback
...
def run_ensemble(briefing: str, odds: dict = None,
                 match_data: dict = None) -> dict | None:
    """Main entry point: run the adaptive 3-phase ensemble.

    match_data (optional) enables the non-LLM quant Poisson voter — uses xG
    profiles + league home-advantage prior to add a 7th panel vote.
    ...
    # Inject the quant Poisson voter as a 7th panel member when match_data is available.
    if match_data:
        league = match_data.get("league", "")
        home_adv = HOME_ADVANTAGE_BY_LEAGUE.get(league, 0.10)
        q_result = quant_poisson_result(match_data, home_advantage=home_adv)
        if q_result:
            results.append(q_result)
            logger.info("Quant Poisson: injected as 7th voter (league=%s, home_adv=%.2f)",
                        league, home_adv)
