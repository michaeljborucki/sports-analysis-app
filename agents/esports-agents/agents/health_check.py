...
import requests
from config import (
    ODDS_API_KEY, ODDSPAPI_API_KEY, OPENROUTER_API_KEY,
    ODDS_API_BASE, ODDSPAPI_BASE, OPENROUTER_BASE_URL,
)


def check_oddspapi() -> tuple[bool, str]:
    """Check OddsPapi API key is set."""
    if not ODDSPAPI_API_KEY:
        return False, "OddsPapi API: NO KEY SET"
    return True, "OddsPapi API: key configured"
...
def run_health_check() -> bool:
    ...
    click.echo("\n=== MiroFish Esports Health Check ===\n")

    checks = [
        ("CRITICAL", check_oddspapi),
        ("CRITICAL", check_openrouter),
        ("OPTIONAL", check_odds_api),
    ]
