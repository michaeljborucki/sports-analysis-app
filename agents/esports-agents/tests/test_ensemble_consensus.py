...
from tests.ensemble_fixtures import MOCK_PREDICTION, MOCK_ODDS, MOCK_GAME_CONFIG
...
def test_extract_vote_map_handicap():
    vote = extract_vote(MOCK_PREDICTION, "map_handicap", MOCK_ODDS,
                        bet_slot_fields=BET_SLOT_FIELDS)
    assert vote == "favorite"

def test_extract_vote_total_maps():
    vote = extract_vote(MOCK_PREDICTION, "total_maps", MOCK_ODDS,
                        bet_slot_fields=BET_SLOT_FIELDS)
    assert vote == "over"
...
def test_extract_vote_requires_bet_slot_fields():
    """extract_vote should raise ValueError without bet_slot_fields."""
    import pytest
    with pytest.raises(ValueError, match="bet_slot_fields"):
        extract_vote(MOCK_PREDICTION, "moneyline", MOCK_ODDS)
