...
def run_phase1(briefing: str, system_prompt: str = None) -> tuple[list[dict], float]:
...
def classify_consensus(results: list[dict], odds: dict,
                       bet_slots: list[str] = None,
                       bet_slot_fields: dict = None) -> dict:
...
def _extract_prob_for_slot(parsed: dict, slot: str,
                           primary_prob_field: dict = None,
                           slot_section: dict = None) -> float | None:
...
def reclassify_consensus(results: list[dict], odds: dict,
                         bet_slots: list[str] = None,
                         bet_slot_fields: dict = None) -> dict:
...
def run_phase2(briefing: str, results: list[dict],
               classification: dict, odds: dict,
               system_prompt: str = None) -> tuple[list[dict], float]:
...
def _apply_stability_bonuses(results: list[dict], weights: dict, odds: dict,
                             bet_slots: list[str] = None,
                             primary_prob_field: dict = None,
                             slot_section: dict = None) -> dict:
        ...
        for slot in bet_slots:
            ...
            for r in model_runs:
                p = _extract_prob_for_slot(r["parsed"], slot,
                                           primary_prob_field=primary_prob_field,
...
def _log_all_predictions(results: list[dict], odds: dict, game_label: str,
                         bet_slots: list[str] = None,
                         bet_slot_fields: dict = None,
                         primary_prob_field: dict = None,
                         slot_section: dict = None) -> None:
    ...
    for r in results:
        ...
        for slot in bet_slots:
            ...
            prob = _extract_prob_for_slot(r["parsed"], slot,
                                          primary_prob_field=primary_prob_field,
                                          slot_section=slot_section) or 0.0
...
def build_ensemble_result(results: list[dict], classification: dict,
                          weights: dict, killed_by_challenger: list[str],
                          bet_slots: list[str] = None,
                          prob_fields: dict = None,
                          slot_section: dict = None) -> dict:
    ...
    # Predicted result: majority vote for winner and score (esports format)
    # Also handles legacy predicted_score with numeric home/away
    predicted_results = []
    predicted_scores = []
    for r in results:
        preds = r["parsed"].get("predictions", {})
        pr = preds.get("predicted_result")
        if pr and "winner" in pr:
            predicted_results.append(pr)
        ps = preds.get("predicted_score")
        if ps and "home" in ps and "away" in ps:
            predicted_scores.append(ps)

    if predicted_results:
        # Majority vote on winner and score strings
        winners = [pr["winner"] for pr in predicted_results]
        winner_counter = Counter(winners)
        best_winner = winner_counter.most_common(1)[0][0]
        score_strings = [pr.get("score", "") for pr in predicted_results if pr.get("score")]
        if score_strings:
            score_counter = Counter(score_strings)
            best_score = score_counter.most_common(1)[0][0]
        else:
            best_score = ""
        predictions["predicted_result"] = {"winner": best_winner, "score": best_score}
    elif predicted_scores:
        # Legacy numeric score averaging fallback
        avg_home = round(sum(s["home"] for s in predicted_scores) / len(predicted_scores))
        avg_away = round(sum(s["away"] for s in predicted_scores) / len(predicted_scores))
    ...
    # Kill slots from challenger — use slot_section for generic mapping
...
def run_ensemble(briefing: str, odds: dict = None, game_config=None) -> dict | None:
    """Main entry point: run the adaptive 3-phase ensemble.

    Args:
        briefing: Pre-built briefing string.
        odds: Game odds dict for consensus normalization.
        game_config: Game module (e.g. games.cs2). Must expose config and prompt submodules.

    Returns complete prediction dict or None (never partial).
    Falls back to None if <3 models succeed or no surviving slots.
    """
    if game_config is None:
        raise ValueError("run_ensemble requires a game_config (game module) — no default game")

    # Extract game-specific configuration
    bet_slots = game_config.config.BET_SLOTS
    prob_fields = game_config.config.PROB_FIELDS
    slot_section = game_config.config.SLOT_SECTION
    primary_prob = game_config.config.PRIMARY_PROB_FIELD
    bet_slot_fields = game_config.config.BET_SLOT_FIELDS
    system_prompt = game_config.prompt.SYSTEM_PROMPT

    odds = odds or {}
    weights = load_weights(bet_slots)
    total_cost = 0.0
    total_calls = 0
    phase_reached = 0
    killed_list = []
    ...
    # --- Phase 1: Quick pass ---
    phase_reached = 1
    results, p1_cost = run_phase1(briefing, system_prompt=system_prompt)
    total_cost += p1_cost
    ...
    if has_soft:
        ...
        weights = _apply_stability_bonuses(results, weights, odds,
                                           bet_slots=bet_slots,
                                           primary_prob_field=primary_prob,
                                           slot_section=slot_section)
