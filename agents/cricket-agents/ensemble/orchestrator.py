...
def _log_all_predictions(results: list[dict], odds: dict, game_label: str) -> None:
    ...
    for r in results:
        ...
        for slot in BET_SLOTS:
            ...
            market_prob = implied.get("team_a", 0.5)  # rough fallback
            ...
            try:
                ...
                logger.warning("Failed to log prediction for %s/%s: %s", mk, slot, e)
...
def build_ensemble_result(results: list[dict], classification: dict,
    ...
    # Predicted result: simple average of projected scores, rounded to int
    scores = []
    for r in results:
        pr = r["parsed"].get("predictions", {}).get("predicted_result")
        if not pr:
            continue
        # Support both flat {"batting_first": X} and nested {"projected_scores": {"batting_first": X}}
        ps = pr.get("projected_scores") or pr
        if "batting_first" in ps and "chasing" in ps:
            scores.append(ps)
    if scores:
        avg_batting_first = round(sum(s["batting_first"] for s in scores) / len(scores))
        avg_chasing = round(sum(s["chasing"] for s in scores) / len(scores))
        predictions["predicted_result"] = {"batting_first": avg_batting_first, "chasing": avg_chasing}
    ...
    if confidence_values:
        ...
        for section_key in ["moneyline", "total_runs"]:
