...
def test_build_challenge_prompt_includes_briefing():
    prompt = build_challenge_prompt(
        briefing="MI vs CSK game data...",
        ensemble_predictions={"moneyline": {"team_a_win_prob": 0.58}},
        model_agreement={"moneyline": "5/6 models say team_a, avg edge 7.2%"},
    )
    assert "MI vs CSK" in prompt
...
def test_parse_challenge_kill():
    raw = json.dumps({
        "challenges": [
            ...
            {"bet_type": "total_runs", "verdict": "approve", "reasoning": "fine", "flaw_found": None},
        ]
    })
    result = parse_challenge_response(raw)
    assert result["moneyline"] == "kill"
    assert result["total_runs"] == "approve"
...
def test_run_challenge_failure_approves_all(mock_runner):
    ...
    verdicts, cost = run_challenge(
        briefing="test", ensemble_predictions={}, model_agreement={},
        surviving_slots=["moneyline", "total_runs"],
    )
    assert verdicts["moneyline"] == "approve"
    assert verdicts["total_runs"] == "approve"
    assert cost == 0
