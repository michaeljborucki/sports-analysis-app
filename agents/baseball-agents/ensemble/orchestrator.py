...
PROB_FIELDS = {
    ...
    "team_total_home": ["predicted_score_home"],
...
# Map bet slots to their correct implied probability key
SLOT_IMPLIED_KEY = {
    "moneyline": "ml_home",
    "run_line": "rl_home",
}
...
def _log_all_predictions(results: list[dict], odds: dict, game_label: str) -> None:
    ...
    for r in results:
        ...
        for slot in BET_SLOTS:
            ...
            implied_key = SLOT_IMPLIED_KEY.get(slot)
...
def build_ensemble_result(results: list[dict], classification: dict,
    ...
    # Predicted score: simple average, keep decimal precision
    scores = []
    for r in results:
        ...
        if ps and "home" in ps and "away" in ps:
            scores.append(ps)
    if scores:
        avg_home = round(sum(s["home"] for s in scores) / len(scores), 2)
        avg_away = round(sum(s["away"] for s in scores) / len(scores), 2)
    ...
    SUB_SLOT_FIELDS = {
        ...
        "team_total_home": ["home_total_value"],
        "team_total_away": ["away_total_value"],
    }
    for slot, info in classification.items():
        if info["level"] == "none":
            section_key = SLOT_SECTION.get(slot)
...
def run_ensemble(briefing: str, odds: dict = None, game_label: str = None) -> dict | None:
    ...
    for slot, v in verdicts.items():
        ...
        if verdict == "kill" and v.get("flaw_found"):
            logger.info("    Flaw: %s", v["flaw_found"])
