...
`main.py grade`) so the channel shows the latest record + per-bet-type
...
from config import DATA_DIR, SEASON_RECORD_START_DATE
...
def send_season_notification(through_date: str,
    ...
    """Post a single-message season summary (record + per-bet-type breakdown)
    aggregated over all graded bets up to and including `through_date`.

    Summary keys: bets_filtered, sent, skipped_reason.
    """
    ...
    # Season totals reflect EVERY market we've graded (props + mainline +
    # team totals + segments). Picks alerts use cfg["bet_types"] but grades
    # and season records intentionally show the full picture (2026-04-29).
    df = load_bets()
    mask = (
        (df["date"] >= SEASON_RECORD_START_DATE)
