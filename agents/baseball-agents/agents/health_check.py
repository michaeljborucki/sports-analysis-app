...
from config import (
    ...
    MLB_API_BASE, ODDS_API_BASE, WEATHER_API_BASE,
    OPENROUTER_BASE_URL,
)


def check_mlb_api() -> tuple[bool, str]:
    """Check MLB Stats API is reachable."""
    try:
        resp = requests.get(f"{MLB_API_BASE}/sports/1", timeout=10)
        resp.raise_for_status()
        return True, "MLB Stats API: OK"
    except Exception as e:
        return False, f"MLB Stats API: FAIL ({e})"
