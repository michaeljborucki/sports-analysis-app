...
Two channels, both driven by the same graded-bet list:
  - `discord_grades`  — match-by-match code blocks, date-stamped header.
...
Auto-fires from the results grader, subject to two rules:
  ...
  2. Once a channel has been posted for a date, it stays in the sent log
     keyed on channel-per-date and requires `force=True` to re-send.
"""
import json
import logging
import os
...
from notify.format import split_grade_blocks, format_grade_header, DISCORD_MAX
...
def send_grade_notifications(game_date: str,
    ...
    graded_picks = [b for b in graded.to_dict(orient="records")
                    if b.get("bet_type") in allowed]
    ...
    # Defensive CLV backfill: if a bet is missing close_odds (e.g. grading ran
    # before the closing line was captured), try one more lookup now so the
    # alert includes CLV wherever possible. Persisting is clv-apply's job —
    # this only fills the in-memory copy used for the notification.
    try:
        from tracker import lookup_clv
        for pick in graded_picks:
            close = pick.get("close_odds")
            if close is None or str(close).strip() in ("", "nan", "NaN"):
                try:
                    clv = lookup_clv(pick)
                except Exception as e:
                    logger.debug("lookup_clv failed for %s: %s", pick.get("game"), e)
                    continue
                if clv:
                    pick.update(clv)
    except ImportError:
