...
def _is_upcoming(bet: dict, now: datetime | None = None) -> bool:
    """True if the match hasn't started yet. Bets with no start_time pass through.

    Legacy rows (pre-start_time column) lack the field; we don't want to
    suppress them silently, so they're treated as upcoming. Parsed times
    that are already in the past are dropped.
    """
    raw = bet.get("start_time")
    if raw is None or (isinstance(raw, float) and raw != raw) or str(raw).strip() == "":
        return True
    ct = parse_match_datetime(str(raw))
    if ct is None:
        return True
    return ct > (now or datetime.now(timezone.utc))
...
def send_notifications(game_date: str | None = None,
    ...
    upcoming = [b for b in bets if _is_upcoming(b, now=now)]
    filtered = filter_bets(upcoming, allowed, min_edge=min_edge, min_kelly=min_kelly)

    with _lock:
        sent_log = _load_sent_log()
        already_sent = set(sent_log.get(game_date, []))

    new_bets = filtered if force else [b for b in filtered if _bet_key(b) not in already_sent]
