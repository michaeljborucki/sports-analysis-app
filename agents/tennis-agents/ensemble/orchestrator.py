...
def _abstain_result(reason: str, meta: dict | None = None) -> dict:
    """Ensemble 'ran but found no bets' result — distinct from a hard failure.

    Returning a structured result (empty ``predictions``) instead of ``None``
    tells ``simulate.run_mirofish`` the ensemble completed successfully and
    deliberately chose not to bet. This prevents the costly Plan B fallback
    from running after a consensus abstention or a challenger-kill-all verdict.

    Downstream (``edge.analyze_all_edges``) sees an empty predictions dict and
    produces zero bets — which is the correct outcome.
    """
    return {
        "analyst_assessments": [],
        "predictions": {},
...
def run_phase1(briefing: str) -> tuple[list[dict], float]:
    ...
    # Manual executor management + ``cancel_futures=True`` so if a
    # ``signal.SIGALRM`` timeout fires in the main thread during this block,
    # ``finally`` shuts down quickly (cancels pending futures, does NOT wait
    # for already-running ones) and the ``MatchTimeout`` exception can
    # actually escape. The ``with ThreadPoolExecutor`` pattern would call
    # ``shutdown(wait=True)`` and block for 40-60 min on Phase 2 pools.
...
def run_phase2(briefing: str, results: list[dict],
    ...
    # Same ``cancel_futures`` pattern as Phase 1 — lets SIGALRM escape cleanly
    # when a per-match timeout fires during expansion.
    executor = ThreadPoolExecutor(max_workers=6)
    try:
...
def _log_all_predictions(results: list[dict], odds: dict, game_label: str) -> None:
    ...
    for r in results:
        ...
        for slot in BET_SLOTS:
            ...
            market_prob = implied.get("player_a", 0.5)  # rough fallback
...
def build_ensemble_result(results: list[dict], classification: dict,
    ...
    # Per-field aggregation: median within model (across runs), then median
    # across model-medians. Switched from weighted arithmetic mean to median
    # on 2026-04-23 because averaging systematically compressed majority
    # signal when 1-2 of 6 models dissented — produced internally inconsistent
    # outputs where the aggregated prob implied betting one side while the
    # majority-vote ``predicted_result.winner`` pointed the other way. The
    # challenger correctly flagged those as kills (Lehecka vs Tabilo case,
    # 2026-04-23). Median preserves the 4-of-6 majority probability instead
    # of dragging it toward the 2-model dissent.
    for slot, fields in PROB_FIELDS.items():
        section_key = SLOT_SECTION[slot]
        section = predictions.get(section_key, {})

        for field in fields:
            # Collect per-model median probabilities (across each model's runs).
            model_medians = {}
            for mk in model_keys:
                probs = []
                for r in results:
                    if r["model_key"] == mk:
                        ...
                        if val is not None:
                            try:
                                ...
                                continue
                if probs:
                    model_medians[mk] = statistics.median(probs)

            if model_medians:
                # Median across the per-model medians — robust to 1-2 outlier
                # models without needing reliable per-model weights (which
                # haven't stabilized until we hit 200+ settled bets per model).
                section[field] = round(statistics.median(model_medians.values()), 4)
                # Attach per-model medians for edge.py's individual-conviction
                # fallback (Option B, 2026-04-24). Median-of-medians cancels to
                # ~0.5 when models disagree on direction — the fallback uses
                # these to detect a ≥4-of-6 supermajority with per-model edge.
                section.setdefault("_per_model_medians", {})[field] = {
                    mk: round(v, 4) for mk, v in model_medians.items()
                }

    # Predicted result: use most common winner from models
    winners = []
    for r in results:
        pr = r["parsed"].get("predictions", {}).get("predicted_result", {})
        if pr and "winner" in pr:
            winners.append(pr["winner"])
    if winners:
        predictions["predicted_result"] = {"winner": majority_vote(winners, default=winners[0]),
                                            "score": base_result.get("predictions", {}).get("predicted_result", {}).get("score", "")}
    ...
    if confidence_values:
        ...
        for section_key in ["moneyline", "game_handicap", "total_games"]:
...
def run_ensemble(briefing: str, odds: dict = None) -> dict | None:
    ...
    if not surviving_slots:
        logger.info("No surviving bet slots after consensus — abstaining (no Plan B fallback)")
        return _abstain_result(
            "no_consensus",
            {"phase_reached": phase_reached, "total_calls": total_calls,
    ...
    for slot, verdict in verdicts.items():
        logger.info("  Challenger verdict: %s → %s", slot, verdict)

    killed_list = [slot for slot, v in verdicts.items() if v == "kill"]
    surviving_after_challenge = [s for s in surviving_slots if s not in killed_list]

    if not surviving_after_challenge:
        logger.info("All bets killed by challenger — abstaining (no Plan B fallback)")
        return _abstain_result(
            "challenger_killed_all",
            {"phase_reached": 3, "total_calls": total_calls,
             "cost_usd": round(total_cost, 4),
             "killed": killed_list, "challenger_verdicts": verdicts},
        )
