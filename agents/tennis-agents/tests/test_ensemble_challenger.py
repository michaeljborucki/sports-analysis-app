...
def test_build_challenge_prompt_includes_briefing():
    prompt = build_challenge_prompt(
        briefing="Djokovic vs Alcaraz match data...",
        ensemble_predictions={"moneyline": {"player_a_win_prob": 0.58}},
        model_agreement={"moneyline": "5/6 models say player_a, avg edge 7.2%"},
    )
    assert "Djokovic vs Alcaraz" in prompt
...
def test_parse_challenge_kill():
    raw = json.dumps({
        "challenges": [
            ...
            {"bet_type": "total_games", "verdict": "approve", "reasoning": "fine", "flaw_found": None},
        ]
    })
    result = parse_challenge_response(raw)
    assert result["moneyline"] == "kill"
    assert result["total_games"] == "approve"
...
def test_run_challenge_failure_approves_all(mock_runner):
    ...
    verdicts, cost = run_challenge(
        briefing="test", ensemble_predictions={}, model_agreement={},
        surviving_slots=["moneyline", "total_games"],
    )
    assert verdicts["moneyline"] == "approve"
    assert verdicts["total_games"] == "approve"
    assert cost == 0
