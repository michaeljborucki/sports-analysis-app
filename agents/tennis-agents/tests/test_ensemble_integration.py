...
def _make_mock_runner():
    """Create a mock that returns different model keys per call."""
    models = iter(["kimi", "claude", "gpt4o", "gemini", "deepseek", "maverick"] * 10)

    def mock_fn(model_key, model_id, briefing, temperature,
...
@patch("ensemble.orchestrator.run_single_model", side_effect=_make_mock_runner())
@patch("ensemble.orchestrator.run_challenge",
       return_value=({"moneyline": "approve", "game_handicap": "approve", "total_games": "approve"}, 0.05))
@patch("ensemble.orchestrator.get_panel_models", return_value=[
...
def test_run_mirofish_uses_ensemble(mock_models, mock_challenge, mock_runner, mock_log):
